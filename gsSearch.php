...
class gsSearch extends gsAPI {
	...
	public function getUniqueID() {
		if (!$this->id || $this->changed) {
			...
			$query_str = preg_replace("/[\s]{2,}/", " ",$query_str); //replace multiple spaces
			$query_str = str_replace(array("{", "}", "<", ">", "@", "$", "%", "~", "#", "*", "|", "/", "_", ";", "^"), "", $query_str); //remove stupid characters
			$query_str = preg_replace("/([\!\?\.\,])[\!\?\.\,]+/", "{1}", $query_str); //remove multiple !?,. characters
